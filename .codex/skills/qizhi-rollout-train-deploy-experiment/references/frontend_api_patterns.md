...
- Workspace discovery:
  ...
  - use `data.routes[name=userWorkspaceList].routes[]` to enumerate the current account's visible workspaces
  - each route entry carries the workspace display label and target `ws-*` id, which can be mapped back into qz config aliases such as `"专项"` / `"分布式"`

## 2.1 Train spec lookup for qz fallback
- The train-job spec lookup path is:
  - `POST /api/v1/resource_prices/logic_compute_groups/`
  - payload includes:
    - `workspace_id`
    - `logic_compute_group_id`
    - `schedule_config_type: "SCHEDULE_CONFIG_TYPE_TRAIN"`
- The response `data.lcg_resource_spec_prices[]` contains the train quota ids used as qz `spec_id`.
- This is the correct source for CPT / train-job fallback specs; do not reuse notebook `SCHEDULE_CONFIG_TYPE_DSW` pricing for train submit.

## 2.2 qz config generation rules
- For the current multi-workspace fallback workflow, `scripts/setup_qz_special_config.py` should render both:
  - `"专项" = "ws-..."`
  - `"分布式" = "ws-..."`
- The generated `[pools]` should include:
  - extra `专项` H200 pools
  - `分布式` H200 pools
  - `分布式` H100 fallback pools
- TOML gotcha:
  - non-ASCII workspace aliases must be quoted keys
  - otherwise `qz pools` / `qz avail` can fail on parse before any capacity scan happens

## 2.3 Duplicate-job winner workflow
- For same-goal fallback retries, keep the old queued job id and the new fallback job id in the same monitor loop.
- The first job that reaches real `running` is the winner.
- After winner selection, request `POST /openapi/v1/train_job/stop` for the loser and continue monitoring only the winner to terminal state.
