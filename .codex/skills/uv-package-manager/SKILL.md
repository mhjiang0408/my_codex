...
## AgentSwarm Hard Rule

In this repository, all local Python-related execution must be managed through `uv`.

- Do not invoke `python`, `python3`, `/usr/bin/python`, `pip`, `pip3`, `python -m pip`,
  `python3 -m pip`, `venv`, or `virtualenv` directly for local workspace execution.
- Do not run bare `pytest`, `ruff`, or `mypy`; use `uv run` forms.
- If `uv` is unavailable, stop and report the issue. Do not fall back to system Python.
- Remote platform skills may document environment-specific exceptions, but those exceptions do
  not change the local AgentSwarm workspace rule.
...
uv add --dev <package>
uv sync
uv run python -m pytest -q tests/unit/path/test_file.py
...
### Repo-Specific Correction Pattern

- If another repository or user explicitly requires an activated shell session, prefer:
...
- Do this even when direct `.venv/bin/python ...` would technically work, because some repos treat
  shell activation as part of the expected runtime contract for environment variables, PATH
  precedence, and debugging reproducibility.

### Repo-Specific Execution Discipline: Never Fall Back To System Python

- If the user or repo policy says this workspace must always use the uv-managed environment,
  treat that as a hard runtime contract.
- In that case, prefer:
...
- Do not use:
  - `python3 ...`
  - bare `pytest` / `ruff` / `mypy`
  - system-site interpreters for smoke runs
- Only use direct `.venv/bin/python ...` when you must inspect the repo-local interpreter itself
  as part of environment diagnosis. For ordinary task execution, keep everything on `uv run`.

### Repo-Specific Repair Pattern: Broken `.venv` With Preserved Python Mapping

- If a repo-local `.venv/bin/python` mapping must stay unchanged and `.venv` must not be deleted,
  first inspect `.venv/pyvenv.cfg`.
- If `home = ...` points at a single-file executable instead of a Python installation `bin`
  directory, fix `home` to the real installation `bin` directory first.
- Symptom cluster for this case:
  - `NotADirectoryError: [Errno 20] Not a directory`
  - `Fatal Python error: error evaluating path`
  - `ModuleNotFoundError: No module named 'encodings'`
- For this repo, a healthy `home` shape is:

```ini
home = /root/.local/share/uv/python/cpython-3.11.13-linux-x86_64-gnu/bin
```

- After fixing `pyvenv.cfg`, validate:

```bash
./.venv/bin/python -c "import sys, encodings, sysconfig; print(sys.executable); print(sys.prefix); print(sys.base_prefix); print(sysconfig.get_path('stdlib'))"
```

- If `./.venv/bin/python` starts but `uv sync` still fails in build isolation and the traceback
  shows `sys._base_executable` pointing to a standalone copied binary path, the second-layer fix
  is to repair that underlying target path so it resolves to a healthy full Python installation.
- In this repo, `uv` build isolation can still fail when the mapped target is:
  - `/inspire/.../.cache/python3.11`
  because `uv` reuses that path as `sys._base_executable` inside `/root/.cache/uv/builds-v0/...`,
  and a standalone copied binary falls back to `/install` for stdlib discovery.
- The minimal safe interpretation is:
  - keep `.venv/bin/python` mapping unchanged,
  - but ensure the mapped target path itself is a symlink to the healthy uv-managed interpreter
    under `/root/.local/share/uv/python/.../bin/python3.11`.
