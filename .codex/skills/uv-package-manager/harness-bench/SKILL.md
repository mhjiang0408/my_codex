...
- If the user explicitly narrows a rollout-success bench remediation to `predict mode` only, freeze the task scope to predict-mode harness/launcher/taxonomy work and do not spend implementation or validation budget on `real mode` compatibility or reruns unless the user asks for that separately.
