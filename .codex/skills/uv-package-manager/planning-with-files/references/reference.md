...
**Solution:** Re-read `.codex/task_plan.md` before each decision. Goals appear in the attention window.

```
Start of context: [Original goal - far away, forgotten]
...many tool calls...
End of context: [Recently read .codex/task_plan.md - gets ATTENTION!]
```
...
|------|---------|--------------|--------------|
| `.codex/task_plan.md` | Phase tracking, progress | Task start | After completing phases |
| `.codex/findings.md` | Discoveries, decisions | After ANY discovery | After viewing images/PDFs |
| `.codex/progress.md` | Session log, what's done | At breakpoints | Throughout session |
...
## Project Pattern: Rollout Failure Audits

- When the user refines a historical-analysis window mid-task, freeze the final scope explicitly in `.codex/task_plan.md` using absolute dates rather than relative wording like "since yesterday".
- For rollout failure audits, treat repository identity mismatches across `query_id`, record metadata, transcript working directory, and workspace path as a first-class failure family.
- If `query_id` repository and transcript workspace repository disagree, prioritize binding/session contamination hypotheses before blaming query generation quality.
