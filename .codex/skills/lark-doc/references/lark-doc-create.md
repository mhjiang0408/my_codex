...
- 在当前 `lark-cli 1.0.19` 中，`--api-version v2` 的文档创建参数与 v1 不同：

```bash
# v2: 内容参数使用 --content，并显式指定 Markdown 格式
lark-cli docs +create --api-version v2 --as bot \
  --title "代码审查报告" --content @review_summary.md --doc-format markdown

# v1 fallback: 内容参数使用 --markdown
lark-cli docs +create --api-version v1 --as bot \
  --title "代码审查报告" --markdown @review_summary.md
```

- 当一个工作流需要兼容不同 `lark-cli` 版本时，优先尝试 v2；如果 v2 命令失败或
  未返回可用文档 URL，再退回 v1。长 Markdown 报告应使用 `@file`，不要直接把
  全文拼到命令行参数中。
- 如果文档由 bot 创建并将链接发送到群里，发送前应为目标群追加最小必要权限，
  例如只读权限：
...
- 如果自动授权失败或无法确认，仍可发送文档链接，但通知消息必须明确提示读者
  检查文档访问权限；不要把未验证的链接表述为群内已可读取。
