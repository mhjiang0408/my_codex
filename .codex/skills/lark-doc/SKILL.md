...
- 发送群消息使用 `lark-cli im +messages-send --chat-id <oc_...> --markdown '<text>'`
  或 `--content ... --msg-type interactive`。不要使用不存在的
  `lark-cli message send` 子命令；该形式会返回 `unknown command "message"`。
- 如果用户要求“不要卡片，用消息即可”，使用普通消息：
  - 优先 `lark-cli im +messages-send --chat-id <oc_...> --text "$REPORT"`。
  - 不要再使用 `--msg-type interactive` 或卡片 JSON。
- `--idempotency-key` 需要保持短小；过长的 key 可能被 Feishu 返回
  `HTTP 400: field validation failed`，但错误信息不会明确指出 key 过长。
  经验上使用类似 `cod695-0521-p1` 的短 key。
- 如果向指定 `chat_id` 发送消息时返回：
  - `Bot/User can NOT be out of the chat.`
  - 或 `API error: [232010] Operator and chat can NOT be in different tenants.`
- 则优先判定为飞书侧权限 / tenant 边界问题，而不是消息 payload 拼装问题。
- 立即补三项复核：
  - `lark-cli auth status`
  - `lark-cli im chats list --as bot`
  - `lark-cli api GET /open-apis/im/v1/chats/<chat_id> --as user` 与 `--as bot`
- 若 `232010` 在 user/bot 两侧都复现，则当前本机身份与目标群不在同一 tenant；本机无法仅靠重试或修改卡片 JSON 完成发送。
- 若 `lark-cli im chats list --as bot` 为空或不含目标群，则 bot 不在群内；此时 `+messages-send --as bot` 不可作为可执行路径。
- 只有在同 tenant 且 bot 在群中，或存在已配置好的备用通知桥接时，才继续尝试自动发送。
