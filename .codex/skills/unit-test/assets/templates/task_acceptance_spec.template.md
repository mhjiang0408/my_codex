...
task_pytest_commands:
  - PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run python -m pytest -q tests/regression/path/to/test_case.py
task_ruff_commands:
  - uv run python -m ruff check src/path/to/code.py tests/regression/path/to/test_case.py
task_mypy_commands:
  - uv run python -m mypy src/path/to/code.py
