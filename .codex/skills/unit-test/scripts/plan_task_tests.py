...
DEFAULT_REPO_RUFF_COMMAND = "uv run python -m ruff check src tests"
DEFAULT_REPO_MYPY_COMMAND = "uv run python -m mypy src tests .codex/skills"
DEFAULT_REPO_PYTEST_COMMAND = "PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run python -m pytest -q"
...
def _default_repo_status_commands(repository_status: dict[str, Any]) -> list[str]:
    ...
    commands: list[str] = []
    commands.extend(
        _join_command(repo_ruff_paths, "uv run python -m ruff check") or [DEFAULT_REPO_RUFF_COMMAND]
    )
    commands.extend(
        _join_command(repo_mypy_paths, "uv run python -m mypy") or [DEFAULT_REPO_MYPY_COMMAND]
    )
    commands.extend(
        _join_command(
            repo_pytest_targets, "PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run python -m pytest -q"
        )
...
def _load_json_spec(spec_path: Path) -> dict[str, Any]:
    ...
    if not task_pytest_commands:
        task_pytest_commands = _join_command(
            pytest_targets, "PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run python -m pytest -q"
        )
    if not task_pytest_commands:
    ...
    if not task_ruff_commands:
        task_ruff_commands = _join_command(lint_paths, "uv run python -m ruff check")

    task_mypy_commands = _list_of_strings(
        payload.get("task_mypy_commands"), field="task_mypy_commands"
    )
    if not task_mypy_commands:
        task_mypy_commands = _join_command(typecheck_paths, "uv run python -m mypy")
