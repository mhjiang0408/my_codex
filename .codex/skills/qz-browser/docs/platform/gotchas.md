...
- `train_job/tb/by_job` can return HTTP 404 for valid historical jobs even when
  `train_job/detail`, `instance_list`, and `logs/train` still work. For retrospective loss
  analysis, prefer raw logs over TensorBoard assumptions.
- `logs/train/download` returns a `text/plain` attachment stream rather than JSON. That is a
  feature, not a parsing bug: save or parse the body as plain text when reconstructing full logs.
- For read-only historical inspection, the cookie-auth endpoint
  `POST /api/v1/train_job/detail {job_id}` is a practical fallback alongside the bearer-token
  openapi detail endpoint.
