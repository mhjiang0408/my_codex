...
- **Gang-capacity interpretation**: Treat `qz avail --type h200 --nodes 8` as candidate-discovery output rather than an immediate gang-allocation guarantee. The command can still list pools whose current gang-capacity is below the requested node count. For H200 gang jobs, inspect `free_nodes + low_pri_nodes` first; only pools whose effective nodes meet the requested size can currently prove that capacity.
- **Submit safety rule**: When using `qz job create` or `qz deploy create` in this repo, do not set task-failure retention duration / failed-job keepalive duration unless the user explicitly asks for that behavior.
- **Room3-128 preference rule**: In this repo, do not treat `special-h200-room3-128` / `128节点3号机房` as the default preferred pool.
  - For deploys: avoid it entirely unless the user explicitly overrides.
  - For training jobs: avoid it when there is a practical alternative; use it only as an explicit exception rather than a silent default.
